...
trait ProxFS2[F[_]] extends Prox {
  ...
  protected override final def unit: ProxIO[Unit] = Applicative[F](instances).unit

  protected override final def pure[A](value: A): ProxIO[A] = Applicative[F](instances).pure(value)
  ...
  protected override final def raiseError(error: ProxError): ProxIO[Unit] = ApplicativeError[F, Throwable](instances).raiseError(error.toThrowable)

  protected override final def ioMap[A, B](io: ProxIO[A], f: A => B): ProxIO[B] = Applicative[F](instances).map(io)(f)

  protected override final def ioFlatMap[A, B](io: ProxIO[A], f: A => ProxIO[B]): ProxIO[B] = FlatMap[F](instances).flatMap(io)(f)

  protected override final def traverse[A, B](list: List[A])(f: A => ProxIO[B]): ProxIO[List[B]] = Traverse[List].traverse(list)(f)(instances)
  ...
  protected override final def makeResource[A](acquire: ProxIO[A], release: A => ProxIO[Unit]): ProxResource[A] = cats.effect.Resource.make(acquire)(release)(instances)

  protected override final def useResource[A, B](r: ProxResource[A], f: A => ProxIO[B]): ProxIO[B] = r.use(f)(instances)

  protected override final def joinFiber[A](f: ProxFiber[A]): ProxIO[A] = f.joinWithNever(instances)
  ...
  protected override final def foldMonoidStream[A: ProxMonoid](s: ProxStream[A]): ProxIO[A] = {
    implicit val i: Sync[F] with Concurrent[F] = instances
