    ...
    suite("Executing a process")(
      ...
      suite("Output redirection")(
        ...
        test("can redirect output to stream") {
          val process =
            Process("echo", List("Hello world!")) ># ZPipeline.utf8Decode
        ...
        test("can redirect output to stream collected to vector") {
          ...
          assertZIO(program)(
            hasSameElements(List(StringLength(5), StringLength(6)))
          )
        },
      ...
      suite("Redirection ordering")(
        ...
        test("can redirect output first then error finally input to stream") {
          val source = ZStream("Hello").flatMap(s =>
            ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8))
          )
          val process = ((Process(
            "perl",
      ...
      suite("Input redirection")(
        test("can use stream as input") {
          ...
          val process =
            Process("wc", List("-w")) < source ># ZPipeline.utf8Decode
        ...
        test("can be terminated with cancellation") {
          ...
          val program = ZIO.scoped {
            process.start().flatMap { fiber =>
              ZIO.sleep(250.millis) *> fiber.interrupt.unit
            }
          }
      ...
      suite("Customization")(
        test("can change the command") {
          val p1 =
            Process("something", List("Hello", "world")) ># ZPipeline.utf8Decode
        ...
        test("respects the working directory") {
          ZIO.attempt(Files.createTempDirectory("prox")).flatMap {
            tempDirectory =>
              val process =
                (Process("pwd") in tempDirectory) ># ZPipeline.utf8Decode
              val program = process.run().map(_.output.trim)
      ...
      test("double input redirect is illegal") {
        assertZIO(
          typeCheck(
            """val bad = (Process("echo", List("Hello world")) < ZStream("X").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))) < ZStream("Y").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))"""
          )
        )(
          isLeft(anything)
        )
