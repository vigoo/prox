    ...
    suite("Piping processes together")(
      suite("Piping")(
        test("is possible with two") {
          ...
          ) | Process("wc", List("-w"))) ># ZPipeline.utf8Decode
        ...
        test("is customizable with pipes") {
          val customPipe = (s: zstream.ProxStream[Byte]) =>
            s
              .via(
                ZPipeline.fromChannel(
                  (ZPipeline.utf8Decode >>> ZPipeline.splitLines).channel
          ...
          val processGroup = Process("echo", List("This is a test string"))
            .via(customPipe)
            .to(Process("wc", List("-w"))) ># ZPipeline.utf8Decode
          val program = processGroup.run().map(_.output.trim)

          program.map(r => assertTrue(r == "10"))
        },
        test("can be mapped") {
          import zstream.Process.*
          ...
          val processGroup2 =
            processGroup1.map(new ProcessGroup.Mapper[String, Unit] {
              override def mapFirst[
                  P <: Process[zstream.ProxStream[Byte], Unit]
              ](process: P): P =
                process.withCommand(process.command.tail).asInstanceOf[P]

              override def mapInnerWithIdx[
                  P <: UnboundIProcess[zstream.ProxStream[Byte], Unit]
              ](process: P, idx: Int): P =
                process.withCommand(process.command.tail).asInstanceOf[P]

              override def mapLast[P <: UnboundIProcess[String, Unit]](
                  process: P
      ...
      suite("Input redirection")(
        test("can be fed with an input stream") {
          ...
          val stream = ZStream("This is a test string").flatMap(s =>
            ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8))
          )
          val processGroup = (Process("cat") | Process(
            "wc",
            List("-w")
          )) < stream ># ZPipeline.utf8Decode
          val program = processGroup.run().map(_.output.trim)
          ...
          withTempFile { tempFile =>
            val program = for {
              ...
              processGroup = (Process("cat") | Process(
                "wc",
                List("-w")
              )) < tempFile.toPath ># ZPipeline.utf8Decode
      ...
      suite("Error redirection")(
        ...
        test("can redirect each error output to a sink") {
          ...
          val target: zstream.ProxSink[Byte] = ZSink.foreach((byte: Byte) =>
            ZIO
              .attempt(builder.append(byte.toChar))
              .mapError(UnknownProxError.apply)
          )
      ...
      suite("Error redirection customized per process")(
        test(
          ...
          val processGroup = (p1 | p2).customizedPerProcess.errorsToFoldMonoid {
            case p if p == p1 =>
              ZPipeline.utf8Decode >>> ZPipeline.map(s => "P1: " + s)
            case p if p == p2 =>
              ZPipeline.utf8Decode >>> ZPipeline.map(s => "P2: " + s)
          }
          val program = processGroup.run()
        ...
        test(
          ...
          val processGroup = (p1 | p2).customizedPerProcess.errorsToSink {
            case p if p == p1 =>
              ZSink.foreach((byte: Byte) =>
                ZIO
                  .attempt(builder1.append(byte.toChar))
                  .mapError(UnknownProxError.apply)
              )
            case p if p == p2 =>
              ZSink.foreach((byte: Byte) =>
                ZIO
                  .attempt(builder2.append(byte.toChar))
                  .mapError(UnknownProxError.apply)
              )
          }
          val program = processGroup.run()
          ...
          withTempFile { tempFile1 =>
            withTempFile { tempFile2 =>
              ...
              val program = for {
                _ <- processGroup.run()
                contents1 <- ZStream
                  .fromFile(tempFile1, 1024)
                  .via(ZPipeline.utf8Decode)
                  .runFold("")(_ + _)
                  .mapError(UnknownProxError.apply)
                contents2 <- ZStream
                  .fromFile(tempFile2, 1024)
                  .via(ZPipeline.utf8Decode)
                  .runFold("")(_ + _)
                  .mapError(UnknownProxError.apply)
      ...
      suite("Redirection ordering")(
        test(
          ...
          val processGroup =
            (p1 | p2 | p3) < stream ># ZPipeline.utf8Decode !># ZPipeline.utf8Decode
        ...
        test(
          ...
          val p3 = Process("wc", List("-w"))
          val processGroup =
            ((p1 | p2 | p3) !># ZPipeline.utf8Decode) ># ZPipeline.utf8Decode < stream
        ...
        test(
          ...
          val p3 = Process("wc", List("-w"))
          val processGroup =
            ((p1 | p2 | p3) !># ZPipeline.utf8Decode) < stream ># ZPipeline.utf8Decode
      ...
      test("bound process is not pipeable") {
        assertZIO(
          typeCheck(
            """val bad = (Process("echo", List("Hello world")) ># ZPipeline.utf8Decode) | Process("wc", List("-w"))"""
          )
        )(
          isLeft(anything)
        )
