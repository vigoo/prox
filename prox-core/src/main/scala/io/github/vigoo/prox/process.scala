...
trait ProcessModule {
  ...
  trait ProcessResult[+O, +E] {
    ...
    /** Output value of the process, depends on what output redirection was
      * applied
      */
    val output: O

    /** Error output value of the process, depends on what error redirection was
      * applied
      */
    val error: E
  }
  ...
  case class SimpleProcessResult[+O, +E](
      override val exitCode: ProxExitCode,
    ...
    def mapInfo[I2](f: Info => I2): RunningProcess[O, E, I2] =
      new RunningProcess[O, E, I2] {
        override val runningInput: ProxFiber[Unit] =
          RunningProcess.this.runningInput
        override val runningOutput: ProxFiber[O] =
          RunningProcess.this.runningOutput
        override val runningError: ProxFiber[E] =
          RunningProcess.this.runningError
        override val info: I2 = f(RunningProcess.this.info)

        override def isAlive: ProxIO[Boolean] = RunningProcess.this.isAlive

        override def kill(): ProxIO[ProcessResult[O, E]] =
          RunningProcess.this.kill()

        override def terminate(): ProxIO[ProcessResult[O, E]] =
          RunningProcess.this.terminate()

        override def waitForExit(): ProxIO[ProcessResult[O, E]] =
          RunningProcess.this.waitForExit()
      }
  }
  ...
  trait Process[O, E] extends ProcessLike with ProcessConfiguration {
    ...
    /** Starts the process asynchronously and returns the [[RunningProcess]]
      * interface for it
      *
      * This is the most advanced way to start processes. See [[start]] and
      * [[run]] as alternatives.
      *
      * @param runner
      ...
      */
    def startProcess[Info]()(implicit
        runner: ProcessRunner[Info]
    ): ProxIO[RunningProcess[O, E, Info]] =
      runner.startProcess(this)

    /** Starts the process asynchronously and returns a closeable fiber
      * representing it
      *
      * Joining the fiber waits for the process to be terminated. Canceling the
      * fiber terminates the process normally (with SIGTERM).
      *
      * @param runner
      *   The process runner to be used
      * @return
    ...
    /** Starts the process asynchronously and blocks the execution until it is
      * finished
      *
      * @param runner
      *   The process runner to be used
...
//    override type Self <: ProcessConfiguration
  ...
  object Process {
    ...
    type UnboundIProcess[O, E] = Process[O, E]
      with RedirectableInput[Process[O, E]]

    /** Process with unbound output stream */
    type UnboundOProcess[E] = Process[Unit, E]
      with RedirectableOutput[Process[*, E]]

    /** Process with unbound error stream */
    type UnboundEProcess[O] = Process[O, Unit]
      with RedirectableError[Process[O, *]]
    ...
    case class ProcessImplIOE[O, E](
      ...
      override protected def selfCopy(
          command: String,
          arguments: List[String],
          workingDirectory: Option[Path],
          environmentVariables: Map[String, String],
    ...
    case class ProcessImplIO[O](
      ...
      override def connectError[R <: OutputRedirection, RE](target: R)(implicit
          outputRedirectionType: OutputRedirectionType.Aux[R, RE]
      ): ProcessImplIOE[O, RE] =
        ProcessImplIOE(
          command,
          arguments,
    ...
    case class ProcessImplI(
      ...
      def connectOutput[R <: OutputRedirection, RO](target: R)(implicit
          outputRedirectionType: OutputRedirectionType.Aux[R, RO]
      ): ProcessImplIO[RO] =
        ProcessImplIO(
          command,
    ...
    /** Defines a process to be executed
      *
      * The process by default uses the standard input, output and error
      * streams.
      *
      * @param command
