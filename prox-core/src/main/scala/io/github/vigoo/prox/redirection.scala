...
trait RedirectionModule {
  ...
  trait RedirectableErrors[+P[_] <: ProcessGroup[_, _]] {
    ...
    lazy val customizedPerProcess: RedirectableErrors.CustomizedPerProcess[P] =
      new RedirectableErrors.CustomizedPerProcess[P] {
        override def connectErrors[R <: GroupErrorRedirection, OR <: OutputRedirection, E](target: R)
                                                                                          (implicit groupErrorRedirectionType: GroupErrorRedirectionType.Aux[R, OR, E],
                                                                                           outputRedirectionType: OutputRedirectionType.Aux[OR, E]): P[E] =
    ...
    /**
      ...
      */
    def connectErrors[R <: GroupErrorRedirection, OR <: OutputRedirection, E](target: R)
                                                                             (implicit groupErrorRedirectionType: GroupErrorRedirectionType.Aux[R, OR, E],
                                                                              outputRedirectionType: OutputRedirectionType.Aux[OR, E]): P[E]
  ...
  object GroupErrorRedirectionType {
    ...
    implicit def groupErrorRedirectionTypeOfStdErr: Aux[AllToStdErr, StdOut, Unit] = new GroupErrorRedirectionType[AllToStdErr] {
      override type Out = Unit
      override type OutputR = StdOut

      override def toOutputRedirectionType(redir: AllToStdErr, process: Process[_, _]): StdOut = StdOut()
    }

    implicit def groupErrorRedirectionTypeOfFile: Aux[AllToFile, OutputFile, Unit] = new GroupErrorRedirectionType[AllToFile] {
      override type Out = Unit
      override type OutputR = OutputFile

      override def toOutputRedirectionType(redir: AllToFile, process: Process[_, _]): OutputFile = OutputFile(redir.pathFn(process), redir.append)
    }

    implicit def groupErrorRedirectionTypeOfStreamThroughPipe[O, OR]: Aux[AllCapturedThroughPipe[O, OR], OutputStreamThroughPipe[O, OR], OR] = new GroupErrorRedirectionType[AllCapturedThroughPipe[O, OR]] {
      override type Out = OR
      override type OutputR = OutputStreamThroughPipe[O, OR]

      override def toOutputRedirectionType(redir: AllCapturedThroughPipe[O, OR], process: Process[_, _]): OutputStreamThroughPipe[O, OR] =
        OutputStreamThroughPipe(redir.pipeFn(process), redir.runner, redir.chunkSize)
    }

    implicit val groupErrorRedirectionTypeOfStreamToSink: Aux[AllCapturedToSink, OutputStreamToSink, Unit] = new GroupErrorRedirectionType[AllCapturedToSink] {
      override type Out = Unit
      override type OutputR = OutputStreamToSink

      override def toOutputRedirectionType(redir: AllCapturedToSink, process: Process[_, _]): OutputStreamToSink =
