...
trait SyntaxModule {
  ...
  /** Extension methods for unbound processes enabling the creation of process
    * groups
    */
  implicit class ProcessPiping(process: Process.UnboundProcess) {
    ...
    /** Attaches the output of this process to an other process' input with a
      * custom channel
      *
      ...
      *   Pipe between the two processes
      * @return
      *   Returns a syntax helper trait that has a [[PipeBuilderSyntax.to]]
      *   method to finish the construction
      */
    def via(
        channel: ProxPipe[Byte, Byte]
    ): PipeBuilderSyntax[ProcessGroup.ProcessGroupImpl] =
      new PipeBuilderSyntax(
        new PipeBuilder[ProcessGroup.ProcessGroupImpl] {
          override def build(
              other: Process.UnboundProcess,
              channel: ProxPipe[Byte, Byte]
          ): ProcessGroup.ProcessGroupImpl =
  ...
  trait PipeBuilder[P] {
    def build(other: Process.UnboundProcess, channel: ProxPipe[Byte, Byte]): P
  }

  class PipeBuilderSyntax[P](
      builder: PipeBuilder[P],
      channel: ProxPipe[Byte, Byte]
  ) {
  ...
  implicit class ProcessStringContextIO(ctx: StringContext) {
    ...
    def proc(args: Any*): Process.ProcessImpl = {
      ...
      val parts =
        staticParts.zipAll(injectedParts, Left(""), Right("")).flatMap {
          case (a, b) => List(a, b)
        }
      val words = parts
        .flatMap {
      ...
      words match {
        ...
        case Nil =>
          throw new IllegalArgumentException(
            s"The proc interpolator needs at least a process name"
          )
      }
    }
  }
