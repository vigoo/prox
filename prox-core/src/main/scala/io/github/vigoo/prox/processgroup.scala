...
trait ProcessGroupModule {
  ...
  trait ProcessGroupConfiguration[O, E] extends ProcessLikeConfiguration {
    ...
    object ConfigApplication {
      def apply[P <: ProcessLikeConfiguration]: ConfigApplication[P] = new ConfigApplication[P]
    }
  ...
  object ProcessGroup {
    ...
    case class ProcessGroupImplIO[O](override val firstProcess: Process.UnboundEProcess[ProxStream[Byte]],
      ...
      override def connectErrors[R <: GroupErrorRedirection, OR <: OutputRedirection, E](target: R)
                                                                                        (implicit groupErrorRedirectionType: GroupErrorRedirectionType.Aux[R, OR, E],
                                                                                         outputRedirectionType: OutputRedirectionType.Aux[OR, E]): ProcessGroupImplIOE[O, E] = {
    ...
    case class ProcessGroupImplIE[E](override val firstProcess: Process[ProxStream[Byte], E],
      ...
      override def connectOutput[R <: OutputRedirection, RO](target: R)(implicit outputRedirectionType: OutputRedirectionType.Aux[R, RO]): ProcessGroupImplIOE[RO, E] = {
        ProcessGroupImplIOE(
          firstProcess,
          innerProcesses,
    ...
    case class ProcessGroupImplOE[O, E](override val firstProcess: Process.UnboundIProcess[ProxStream[Byte], E],
      ...
      override def connectInput(source: InputRedirection): ProcessGroupImplIOE[O, E] = {
        ProcessGroupImplIOE(
    ...
    case class ProcessGroupImplI(override val firstProcess: Process.UnboundEProcess[ProxStream[Byte]],
      ...
      override def connectOutput[R <: OutputRedirection, RO](target: R)(implicit outputRedirectionType: OutputRedirectionType.Aux[R, RO]): ProcessGroupImplIO[RO] = {
        ProcessGroupImplIO(
          firstProcess,
          innerProcesses,
          lastProcess.connectOutput(target),
      ...
      override def connectErrors[R <: GroupErrorRedirection, OR <: OutputRedirection, E](target: R)
                                                                                        (implicit groupErrorRedirectionType: GroupErrorRedirectionType.Aux[R, OR, E],
                                                                                         outputRedirectionType: OutputRedirectionType.Aux[OR, E]): ProcessGroupImplIE[E] = {
        val origs = originalProcesses.reverse.toVector
        ProcessGroupImplIE(
          firstProcess.connectError(groupErrorRedirectionType.toOutputRedirectionType(target, origs.head)),
          innerProcesses.zipWithIndex.map { case (p, idx) => p.connectError(groupErrorRedirectionType.toOutputRedirectionType(target, origs(idx + 1))) },
    ...
    case class ProcessGroupImplO[O](override val firstProcess: Process.UnboundIEProcess[ProxStream[Byte]],
      ...
      override def connectInput(source: InputRedirection): ProcessGroupImplIO[O] = {
        ProcessGroupImplIO(
          firstProcess.connectInput(source),
          innerProcesses,
          lastProcess,
      ...
      override def connectErrors[R <: GroupErrorRedirection, OR <: OutputRedirection, E](target: R)
                                                                                        (implicit groupErrorRedirectionType: GroupErrorRedirectionType.Aux[R, OR, E],
                                                                                         outputRedirectionType: OutputRedirectionType.Aux[OR, E]): ProcessGroupImplOE[O, E] = {
        val origs = originalProcesses.reverse.toVector
        ProcessGroupImplOE(
          firstProcess.connectError(groupErrorRedirectionType.toOutputRedirectionType(target, origs.head)),
          innerProcesses.zipWithIndex.map { case (p, idx) => p.connectError(groupErrorRedirectionType.toOutputRedirectionType(target, origs(idx + 1))) },
    ...
    case class ProcessGroupImplE[E](override val firstProcess: Process.UnboundIProcess[ProxStream[Byte], E],
      ...
      override def connectOutput[R <: OutputRedirection, RO](target: R)(implicit outputRedirectionType: OutputRedirectionType.Aux[R, RO]): ProcessGroupImplOE[RO, E] = {
        ProcessGroupImplOE(
          firstProcess,
          innerProcesses,
          lastProcess.connectOutput(target),
      ...
      override def connectInput(source: InputRedirection): ProcessGroupImplIE[E] = {
        ProcessGroupImplIE(
          firstProcess.connectInput(source),
          innerProcesses,
          lastProcess,
    ...
    case class ProcessGroupImpl(override val firstProcess: Process.UnboundIEProcess[ProxStream[Byte]],
      ...
      override def connectErrors[R <: GroupErrorRedirection, OR <: OutputRedirection, E](target: R)
                                                                                        (implicit groupErrorRedirectionType: GroupErrorRedirectionType.Aux[R, OR, E],
                                                                                         outputRedirectionType: OutputRedirectionType.Aux[OR, E]): ProcessGroupImplE[E] = {
        val origs = originalProcesses.reverse.toVector
        ProcessGroupImplE(
          firstProcess.connectError(groupErrorRedirectionType.toOutputRedirectionType(target, origs.head)),
          innerProcesses.zipWithIndex.map { case (p, idx) => p.connectError(groupErrorRedirectionType.toOutputRedirectionType(target, origs(idx + 1))) },
      ...
      override def connectOutput[R <: OutputRedirection, RO](target: R)(implicit outputRedirectionType: OutputRedirectionType.Aux[R, RO]): ProcessGroupImplO[RO] = {
        ProcessGroupImplO(
          firstProcess,
          innerProcesses,
          lastProcess.connectOutput(target),
