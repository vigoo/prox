...
trait ProcessRunnerModule {
  ...
  trait ProcessRunner[Info] {
    ...
    /** Starts the process asynchronously and returns the [[RunningProcess]]
      * interface for it
      *
      * @param process
      ...
      */
    def startProcess[O, E](
        process: Process[O, E]
    ): ProxIO[RunningProcess[O, E, Info]]

    /** Starts the process asynchronously and returns a managed fiber
      * representing it
      *
    ...
    def start[O, E](
        process: Process[O, E]
    ): ProxResource[ProxFiber[ProcessResult[O, E]]] = {
      val run = startFiber(bracket(startProcess(process)) { runningProcess =>
    ...
    /** Starts a process group asynchronously and returns an interface for them
      ...
      */
    def startProcessGroup[O, E](
        processGroup: ProcessGroup[O, E]
    ): ProxIO[RunningProcessGroup[O, E, Info]]

    /** Starts the process group asynchronously and returns a managed fiber
      * representing it
      *
      * Joining the fiber means waiting until the process gets terminated.
      * Cancelling the fiber terminates the process.
  ...
  class JVMRunningProcess[O, E, +Info <: JVMProcessInfo](
      val nativeProcess: JvmProcess,
    ...
    def kill(): ProxIO[ProcessResult[O, E]] =
      effect(nativeProcess.destroyForcibly(), FailedToDestroy.apply).flatMap(
        _ => waitForExit()
      )

    def terminate(): ProxIO[ProcessResult[O, E]] =
      effect(nativeProcess.destroy(), FailedToDestroy.apply).flatMap(_ =>
        waitForExit()
      )

    def waitForExit(): ProxIO[ProcessResult[O, E]] = {
      for {
        exitCode <- blockingEffect(
          nativeProcess.waitFor(),
          FailedToWaitForExit.apply
        )
        _ <- runningInput.join
  ...
  /** Default implementation of [[RunningProcessGroup]] using the Java process
    * API
    */
  class JVMRunningProcessGroup[O, E, +Info <: JVMProcessInfo](
      runningProcesses: Map[Process[Unit, Unit], RunningProcess[_, E, Info]],
      override val runningOutput: ProxFiber[O]
    ...
    def kill(): ProxIO[ProcessGroupResult[O, E]] =
      traverse(runningProcesses.values.toList)(_.kill().map(_ => ())).flatMap(
        _ => waitForExit()
      )

    def terminate(): ProxIO[ProcessGroupResult[O, E]] =
      traverse(runningProcesses.values.toList)(_.terminate().map(_ => ()))
        .flatMap(_ => waitForExit())
    ...
    def waitForExit(): ProxIO[ProcessGroupResult[O, E]] =
      for {
        ...
        exitCodes = results.map { case (proc, result) =>
          proc -> result.exitCode
        }.toMap
        errors = results.map { case (proc, result) =>
          proc -> result.error
        }.toMap
  ...
  abstract class JVMProcessRunnerBase[Info <: JVMProcessInfo]
    ...
    override def startProcess[O, E](
        process: Process[O, E]
    ): ProxIO[RunningProcess[O, E, Info]] = {
      val builder = withEnvironmentVariables(
        process,
      ...
      builder.redirectOutput(
        ouptutRedirectionToNative(process.outputRedirection)
      )
      ...
      for {
        ...
        nativeOutputStream <- effect(
          nativeProcess.getInputStream,
          UnknownProxError.apply
        )
        nativeErrorStream <- effect(
          nativeProcess.getErrorStream,
          UnknownProxError.apply
        )
    ...
    private def connectAndStartProcesses[E](
      ...
      startProcess(
        firstProcess.connectInput(
          InputStream(previousOutput, flushChunks = false)
        )
      ).flatMap { first =>
        first.runningOutput.join.flatMap { firstOutput =>
          ...
          remainingProcesses match {
            case nextProcess :: rest =>
              connectAndStartProcesses(
                nextProcess,
                firstOutput,
                rest,
                updatedStartedProcesses
              )
            case Nil =>
    ...
    override def startProcessGroup[O, E](
        processGroup: ProcessGroup[O, E]
    ): ProxIO[RunningProcessGroup[O, E, Info]] =
      for {
        ...
        innerResult <-
          if (processGroup.innerProcesses.isEmpty) {
        ...
        last <- startProcess(
          processGroup.lastProcess.connectInput(
            InputStream(lastInput, flushChunks = false)
          )
        )
        runningProcesses = processGroup.originalProcesses.reverse
          .zip((first :: inner) :+ last)
          .toMap
    ...
    private def runInputStream[O, E](
        process: Process[O, E],
  ...
  object JVMProcessRunnerBase {
    ...
    def withEnvironmentVariables[O, E](
        process: Process[O, E],
        builder: ProcessBuilder
    ): ProcessBuilder = {
