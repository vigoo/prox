  ...
  override val spec =
    suite("Executing a process")(
      ...
      suite("Output redirection")(
        testM("can redirect output to a file") {
        ...
        testM("can redirect output to append a file") {
          withTempFile { tempFile =>
        ...
        testM("can redirect output to stream") {
        ...
        testM("can redirect output to stream folding monoid") {
        ...
        testM("can redirect output to stream collected to vector") {
        ...
        testM("can redirect output to stream and fold it") {
          val process = Process("echo", List("Hello\nworld!")).foldOutput(
      ...
      suite("Error redirection")(
        testM("can redirect error to a file") {
          withTempFile { tempFile =>
        ...
        testM("can redirect error to append a file") {
          withTempFile { tempFile =>
        ...
        testM("can redirect error to stream") {
        ...
        testM("can redirect error to stream folding monoid") {
        ...
        testM("can redirect error to stream collected to vector") {
          case class StringLength(value: Int)
        ...
        testM("can redirect error to stream and ignore it's result") {
        ...
        testM("can redirect error to stream and fold it") {
          val process = Process("perl", List("-e", "print STDERR 'Hello\nworld!'")).foldError(
            ZTransducer.utf8Decode >>> ZTransducer.splitLines,
            Vector.empty,
            (l: Vector[Option[Char]], s: String) => l :+ s.headOption
      ...
      suite("Redirection ordering")(
        testM("can redirect first input and then error to stream") {
        ...
        testM("can redirect error first then output to stream") {
        ...
        testM("can redirect output first then error to stream") {
          val process = (Process("perl", List("-e", """print STDOUT Hello; print STDERR World""".stripMargin)) ># ZTransducer.utf8Decode) !># ZTransducer.utf8Decode
          val program = process.run().map(r => r.output + r.error)
        ...
        testM("can redirect output first then error finally input to stream") {
        ...
        testM("can redirect output first then input finally error to stream") {
          val source = ZStream("Hello").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
          val process = ((Process("perl", List("-e", """my $str = <>; print STDOUT "$str"; print STDERR World""".stripMargin))
            ># ZTransducer.utf8Decode)
        ...
        testM("can redirect input first then error finally output to stream") {
          val source = ZStream("Hello").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
          val process = ((Process("perl", List("-e", """my $str = <>; print STDOUT "$str"; print STDERR World""".stripMargin))
            < source)
      ...
      suite("Input redirection")(
        ...
        testM("can use stream as input flushing after each chunk") {
          val source = ZStream("This ", "is a test", " string").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
          val process = (Process("wc", List("-w")) !< source) ># ZTransducer.utf8Decode
          val program = process.run().map(_.output.trim)
      ...
      suite("Customization")(
        ...
        testM("can change the arguments") {
        ...
        testM("respects the working directory") {
        ...
        testM("is customizable with environment variables") {
        ...
        testM("is customizable with excluded environment variables") {
          val process = (Process("sh", List("-c", "echo \"Hello $TEST1! I am $TEST2!\""))
            `with` ("TEST1" -> "world")
            `with` ("TEST2" -> "prox")
        ...
        testM("is customizable with environment variables output is bound") {
        ...
        testM("is customizable with environment variables if input is bound") {
          val source = ZStream("This is a test string").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
        ...
        testM("is customizable with environment variables if error is bound") {
          val source = ZStream("This is a test string").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
          val process = ((Process("sh", List("-c", "echo \"Hello $TEST1! I am $TEST2!\"")) !># ZTransducer.utf8Decode)
            `with` ("TEST1" -> "world")
            `with` ("TEST2" -> "prox")) ># ZTransducer.utf8Decode
        ...
        testM("is customizable with environment variables if input and output are bound") {
          val source = ZStream("This is a test string").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
        ...
        testM("is customizable with environment variables if input and error are bound") {
          val source = ZStream("This is a test string").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
          val process = (((Process("sh", List("-c", "cat > /dev/null; echo \"Hello $TEST1! I am $TEST2!\"")) < source) !># ZTransducer.utf8Decode)
            `with` ("TEST1" -> "world")
            `with` ("TEST2" -> "prox")) ># ZTransducer.utf8Decode
        ...
        testM("is customizable with environment variables if output and error are bound") {
        ...
        testM("is customizable with environment variables if everything is bound") {
          val source = ZStream("This is a test string").flatMap(s => ZStream.fromIterable(s.getBytes(StandardCharsets.UTF_8)))
